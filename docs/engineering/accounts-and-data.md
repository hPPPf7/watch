...
- 若好友在同一天同作品（影集／動畫為同季同集）已有紀錄，這次同步整筆不成立，不是略過該好友繼續存。介面須指出衝突好友並保留目前選擇，使用者可取消勾選後重新送出，與其餘好友一起紀錄。
...
- 成為好友表示彼此同意直接新增一起觀看的同步紀錄；維持既有好友邀請的同意流程，不增加逐筆觀看紀錄的接受步驟。好友頁需在送出／接受邀請前說明此授權範圍。
- 為處理同步衝突，允許對「這次選取且目前仍是好友」的人，回報其在指定作品／季集／觀看日期是否有衝突。請求者可能因此推測好友已有相關紀錄，這是已接受的產品取捨；不得為隱藏此推測而取消必要提醒或改成逐筆接受。
- 衝突回應僅包含必要錯誤代碼與衝突好友 ID，ID 順序須固定，不按紀錄來源分組；不得額外回傳未分享紀錄的觀看日期、紀錄 ID、其他參與者、OWNER／來源或其他詳細內容。好友姓名由原本可見的好友資料取得，不增加查詢。
- 有衝突時不得替好友刪除、改期或覆蓋原紀錄。是否能調整，須由好友自行查看；需要修改原紀錄時由該筆紀錄 OWNER 操作。
...
- 電影、影集／動畫詳情觀看紀錄讀取失敗時保留同帳號、同作品（影集另須同季）最近成功的資料，顯示錯誤與手動重試；沒有成功資料時不得顯示「尚未建立觀看紀錄」、零筆或一直維持載入中。切換作品或季度不能沿用上一個範圍的紀錄，不得為此新增常駐輪詢。
- 帳戶暱稱與好友清單必須區分讀取中、成功空值及失敗。失敗保留適用的成功資料並提供手動重試；首次暱稱讀取失敗可顯示 Google 名稱參考，但須明示失敗且不開放依未知資料編輯。不得用 fallback 值標記成成功讀取，也不因顯示載入狀態增加背景查詢。
