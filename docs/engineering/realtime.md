...
- 同一分頁的頁首、清單、詳細資料視窗共用一條 watchlist SSE；連線正常不另行輪詢。詳細資料停止 20 秒輪詢，斷線時才以五分鐘 fallback 補查。日期／好友編輯期間保留共用連線，只延後資料刷新；收起時僅在收到事件、重連或 fallback 到期等更新訊號後合併補查一次，不因開關編輯器重新訂閱或無條件查詢。伺服器解除訂閱必須可重複呼叫；休眠前的請求完成後，仍須交付恢復操作後排入的更新。
