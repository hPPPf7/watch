...
- 卡片保留既有平面背景，不增加陰影或漸層層次；滑入僅輕微位移且尊重減少動態偏好。載入佔位不持續旋轉或閃爍。集數進度只使用既有已知總集數與一致的觀看計數，載入中、無法確認最新資料或分母無效時隱藏，不新增查詢。

- 卡片採 B 精簡排版：有效集數進度與下一集／集名或已追完狀態並列保留，計數與「已知總集數」同列、細條置底；漏集、資料不完整、載入及上映倒數等重要狀態維持原文，不因精簡而隱藏。不得固定內容高度造成文字裁切。

- 卡片進度分母沿用各季已建立資料的集數加總，可能包含未播出集數，不得標示為「已播出」。標籤使用「已知總集數」，滑鼠提示「總集數可能包含尚未播出的集數」；輔助閱讀文字也須交代此語意，不增加集數查詢。
...
- 桌面下一集快取須連同已驗證的漏集判斷保存於既有清單快取，並核對觀看位置與計數再重用；補看或刪除後交回原掃描重算。舊快取缺少判斷時走原檢查流程，不新增輪詢、API 或資料庫欄位。
