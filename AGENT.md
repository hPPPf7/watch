...
- `已看 X / Y` 的分母統一使用「已播出集數」，不得回退「已知總集數」。資料不完整時保留已看數並標示「已播出待確認」，不畫猜測的進度條。等於目前分母且沒有漏集警告時用綠色；未看完用淡藍色，漏集警告優先用黃色。
...
- 卡片採 B 精簡排版：有效集數進度與下一集／集名或已追完狀態並列保留，計數與對應的集數分母標籤同列、細條置底；漏集、資料不完整、載入及上映倒數等重要狀態維持原文，不因精簡而隱藏。不得固定內容高度造成文字裁切。

- 卡片、詳情視窗及季數選單使用 `episodeTotals.ts` 共用已播出計算。全部已知非特別篇季的集數與播出日期齊全、數量一致時，按台北日期計算；缺季、缺日期、數量不符或已看數超過已播出數時，不使用已知總數替代。不得把未載入季當成零、只憑季數推定已播畢或截斷使用者觀看數；數量標籤、提示與輔助閱讀文字須一致。清單只為已有觀看紀錄的作品補查，詳情僅在已登入、開啟且未閒置／編輯時補查；每部逐季、共用四個請求名額，失敗交給既有檢查週期。
...
- 卡片進度配色：未看完用低飽和淡藍、總數全看完用綠色；漏集／集數資料不完整優先用黃色數字及進度條。警告不應單獨阻擋有效進度，未知分母與查詢失敗仍隱藏進度條。同步確認中的過期進度也不得顯示。
...
- 前端集數清單（`tv:<id>:season:<n>`）的 in-memory 快取壽命依作品狀態決定（`resolveSeasonEpisodesClientTtlMs`）：已完結 / 已取消的舊季度用 7 天長快取；近期播畢、含未來或缺少播出日期時仍上限 6 小時，播出中或狀態未知一律用 6 小時短快取。桌面版 renderer 是常駐 session，播出中作品若吃長快取會讓新集數偵測與集數清單卡在舊資料。
...
- 刪除文案必須區分 Watch 站內資料與共用登入帳號；刪除共用帳號會移除共用登入／個人資料，但此端點不清除其他網站的業務資料。
...
- TV 作品前端詳情最多快取六小時；完整季資料播出中六小時、已完結舊季度七天；近期／未來／不完整日期仍上限六小時。桌面已完結詳情改為最多七天（讀取舊 30／90 天快取也須套用上限）。這些期限是回查週期，並非 TMDB 新增資料到各層立即可見的保證；服務端仍沿用播出日感知 TTL，離線／上游失敗不可偽裝為最新。
...
- TMDB 429 必須遵守 Retry-After；作品與季端點共用程序內冷卻，前端共用冷卻以避免原有立即重試再次送出。保持現有四個請求併發上限、來源標示、server-only API key 與不超過六個月的保存限制。官方規範以 https://developer.themoviedb.org/docs/rate-limiting 、https://developer.themoviedb.org/docs/faq 、https://www.themoviedb.org/api-terms-of-use 為準，不能把目前上限當作永久配額。

- 已播出分母使用 `episodeDateCache.ts` 的獨立公開日期摘要（不含使用者紀錄、集名或圖片），在網站與桌面 renderer 的 localStorage 保存；最多 1000 筆／100 萬字元，過期或容量超限會淘汰。季摘要最多六小時；日期全部有效且最後一集播出超過 30 天的季度可保留 30 天，其餘最多六小時。只在共用 loader 寫入回應時設定期限，一般讀取、同步與跨日重算不得續期。取得較新回應時覆蓋；目前季摘要集數不符時禁止沿用舊分母。這是日期計數摘要的期限，不延長完整集名清單的快取。
