...
- `DetailModal` 的觀看紀錄好友選擇器在桌面版使用每列 4 位、最多顯示 2 列的網格，超出 8 位後維持容器內垂直捲動；行動版改為每列 2 位並由外層內容捲動。好友名稱過長時允許截斷，但需保留完整名稱提示。
...
- 月曆頁的月份、月份切換、紀錄篩選、檢視模式與媒體圖例在桌面寬度下需維持同一列；窄視窗可換行，不能為了硬塞單列而產生水平溢出。整組控制項作為 `SiteHeader` 下方的第二層 sticky 工具列，頁面內容捲動時不可跟著離開畫面；sticky 外框需貼齊 Header 下緣，透過工具列本身的緊湊上內距分隔兩列內容，避免留下會透出滾動內容的空隙。
...
- TV / Anime 的集數背景掃描需在清單標題旁顯示精簡狀態：確認中顯示「正在確認更新…」、有未讀的新集數 / 首播提醒時顯示發現更新的作品數，完整掃描且沒有更新才顯示「已是最新」；任一作品暫時無法確認時不可宣稱已是最新。不要混入「已置頂」等實作細節，也不能只顯示觀看紀錄同步完成。清單保持開啟時依播出中集數快取週期（目前 6 小時）自動重查；從閒置恢復時的補查規則見下方「集數狀態掃描」條目，兩處不要各自描述、避免說法兜不起來。
...
- TV 集數狀態掃描（新集數偵測）需從上一輪 `buildStatus()` 實際完成時間起算 `SHORT_DETAIL_TTL_MS`（6 小時），以動態 timeout 排下一輪，不可用固定 6 小時 interval 搭配到期判斷，否則第一個 tick 可能只差數秒而錯過、把間隔拖成 12 小時。使用者從閒置（分頁隱藏或超過 idle 門檻沒有互動）恢復操作時，需強制立刻重新檢查一次，不等排程間隔，且這條強制路徑必須略過瀏覽器 in-memory 與 server TMDB 快取，確實取得最新 detail / season 資料。強制補查跟 revision 同步的 resume 檢查一樣要有冷卻（`RESUME_EPISODE_STATUS_CHECK_COOLDOWN_MS`，直接沿用 `RESUME_REVISION_CHECK_COOLDOWN_MS` 同一個常數），避免使用者短時間內反覆切換分頁/閒置時每次回來都觸發一輪完整掃描；`lastEpisodeStatusCheckAtRef`（只在 `buildStatus()` 真的執行完後才更新，用來重排下一個 6 小時 timeout）與 `lastEpisodeStatusRefreshRequestedAtRef`（只在恢復路徑更新，用來擋恢復冷卻）是兩個分開的時間戳。`episodeStatusCheckInFlightRef` 用來避免固定排程疊加掃描；若恢復補查撞上普通掃描，需在普通掃描完成後緊接一輪強制刷新，避免沿用舊的 in-flight cache promise。guard 卡住的期間也要讓 `episodeStatusRequestIdRef` 前進一格，使舊結果解出時不會被誤套用。
