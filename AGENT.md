...
- 卡片保留既有平面背景，不增加陰影或漸層層次；滑入僅輕微位移且尊重減少動態偏好。載入佔位不持續旋轉或閃爍。集數進度只使用既有已播出總數與一致的觀看計數，資料不完整、載入中或分母無效時隱藏，不新增查詢。

- 卡片採 B 精簡排版：有效集數進度取代一般觀看位置文字，計數與「已播出」同列、細條置底；漏集、資料不完整、載入及上映倒數等重要狀態維持原文，不因精簡而隱藏。不得固定內容高度造成文字裁切。
