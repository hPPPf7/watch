...
- `已看 X / Y` 的分母須明確標示「已播出集數」或「已知總集數」。等於目前分母且沒有漏集警告時用綠色；未看完用淡藍色，漏集警告優先用黃色。
...
- 卡片保留既有平面背景，不增加陰影或漸層層次；滑入僅輕微位移且尊重減少動態偏好。載入佔位不持續旋轉或閃爍。集數進度只使用既有共用資料與一致的觀看計數，載入中、無法確認最新資料或分母無效時隱藏，不新增查詢。

- 卡片採 B 精簡排版：有效集數進度與下一集／集名或已追完狀態並列保留，計數與對應的集數分母標籤同列、細條置底；漏集、資料不完整、載入及上映倒數等重要狀態維持原文，不因精簡而隱藏。不得固定內容高度造成文字裁切。

- 卡片與詳情視窗使用 `episodeTotals.ts` 共用分母計算。全部已知非特別篇季的集數與播出日期齊全、數量一致時，按台北日期計算「已播出集數」；缺季、缺日期、數量不符或已看數超過已播出數時回退「已知總集數」。不得把未載入季當成零、只憑季數推定已播畢或截斷使用者觀看數；數量標籤、提示與輔助閱讀文字須一致。不為分母補查其他季。
...
- 卡片進度配色：未看完用低飽和淡藍、總數全看完用綠色；漏集／集數資料不完整優先用黃色數字及進度條。警告不應單獨阻擋有效進度，未知分母與查詢失敗仍隱藏進度。同步確認中的過期進度也不得顯示。
...
- 前端集數清單（`tv:<id>:season:<n>`）的 in-memory 快取壽命依作品狀態決定（`resolveSeasonEpisodesClientTtlMs`）：已完結 / 已取消用 7 天長快取，播出中或狀態未知一律用 6 小時短快取。桌面版 renderer 是常駐 session，播出中作品若吃長快取會讓新集數偵測與集數清單卡在舊資料。
...
- TV 作品前端詳情最多快取六小時；季資料播出中六小時、已完結七天。桌面已完結詳情改為最多七天（讀取舊 30／90 天快取也須套用上限）。這些期限是回查週期，並非 TMDB 新增資料到各層立即可見的保證；服務端仍沿用播出日感知 TTL，離線／上游失敗不可偽裝為最新。
