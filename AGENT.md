...
- TV / Anime 的集數背景掃描需在清單標題旁顯示精簡狀態：確認中顯示「正在確認更新…」、有未讀的新集數 / 首播提醒時顯示發現更新的作品數，沒有更新則顯示「已是最新」；不要混入「已置頂」等實作細節，也不能只顯示觀看紀錄同步完成。清單保持開啟時依播出中集數快取週期（目前 6 小時）自動重查；從閒置恢復時的補查規則見下方「集數狀態掃描」條目，兩處不要各自描述、避免說法兜不起來。
