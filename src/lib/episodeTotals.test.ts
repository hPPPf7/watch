...
describe("shared episode totals", () => {
 ...
 it("does not infer missing older seasons or unknown dates", () => {
  expect(calculateEpisodeProgress(6,24,[{season_number:1,episode_count:12},{season_number:2,episode_count:12}],n=>n===1?season(12,8):null,"2026-09-11").totalKind).toBe("known");
  const episodes=season(12,8);episodes[0].air_date=null;
  expect(calculateEpisodeProgress(6,12,[{season_number:1,episode_count:12}],()=>episodes,"2026-09-11").totalKind).toBe("known");
 });
 it.each(["duplicates","missing","bad-date","version-mismatch"])("rejects incomplete or inconsistent data: %s", kind => {
  ...
  expect(calculateEpisodeProgress(6,kind==="version-mismatch"?13:12,[{season_number:1,episode_count:12}],()=>episodes,"2026-09-11").totalKind).toBe("known");
 });
 it("never clamps or discards viewing records beyond the aired total", () => {
  expect(calculateEpisodeProgress(10,12,[{season_number:1,episode_count:12}],()=>season(12,8),"2026-09-11")).toEqual({watched:10,total:12,totalKind:"known"});
 });
 ...
 it("notifies all views when another view supplies a season and expires without a request", () => {
  ...
  expect(getSharedEpisodeProgress(87654321,6,12,"2026-09-11").totalKind).toBe("known");
