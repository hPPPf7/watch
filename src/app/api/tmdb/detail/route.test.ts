...
vi.mock("@/server/tmdb/calendarMetadata", () => ({ readCalendarMetadataCacheState: metadata, refreshCalendarMetadataIfTitleNeedsRefresh: background }));
...
const state = (title: string) => ({ payload: { title }, expired: false, legacyTitleDue: false, expiresAt: Date.now() + 86_400_000 });
describe("詳情中文標題退避", () => {
  ...
  it("有效 metadata 中的原文標題不會每次強制重查", async () => {
    metadata.mockResolvedValue(state("English"));
    for (let i = 0; i < 2; i++) expect((await GET(new Request("https://watch.invalid/api/tmdb/detail?type=movie&id=1"))).status).toBe(200);
    expect(detail).not.toHaveBeenCalled();
  });
  it.each([null, state("新標題"), { ...state("English"), expired: true }, { ...state("English"), legacyTitleDue: true }])("到期或已有中文標題會刷新：%j", async state => {
