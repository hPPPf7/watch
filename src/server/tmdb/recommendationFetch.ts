import { extendTmdbCooldown, fetchTmdbWithCooldown, tmdbRetryAfterSeconds, tmdbRetryDelayMs } from "@/server/tmdb/fetchWithCooldown";
...
function mergeCooldown(value: unknown) {
  ...
  if (next.status === 429) extendTmdbCooldown(next.until);
...
function currentCooldown(): Cooldown | null {
  const now = Date.now();
  const local = cooldown && cooldown.until > now ? cooldown : null;
  const sharedSeconds = tmdbRetryAfterSeconds();
  const localSeconds = local ? Math.ceil((local.until - now) / 1000) : 0;
  return sharedSeconds > localSeconds
    ? { until: now + sharedSeconds * 1000, status: 429 }
    : local;
}
function assertLocalAvailability() {
  const state = currentCooldown();
  if (state) throw new RecommendationCooldownError(state);
}
...
async function fail(status: 429 | 502, retryAfter: string | null = null): Promise<never> {
  const next = { until: Date.now() + tmdbRetryDelayMs(retryAfter, status === 429 ? 60_000 : 15_000), status };
...
export async function fetchRecommendationJson<T>(url: string): Promise<T> {
  ...
  try { response = await fetchTmdbWithCooldown(url); }
...
export function recommendationErrorResponse(error: unknown) {
  if (error instanceof RecommendationCooldownError) {
    const current = currentCooldown();
    const state = current && current.until > error.state.until ? current : error.state;
