...
- **已播出集數**：卡片、詳情進度與季數選單統一依 TMDB 播出日期與台北日期計算。例如已看 5 集、已播 6 集、預計共 12 集，進度顯示 **5／6**；不代表各串流平台的即時上架時間。
- **資料待確認**：缺少季度或播出日期時保留已看集數，不把未播出集數混入分母；補齊後自動顯示進度。
...
- 完整集名清單的前端快取：播出中六小時，已完結舊季度七天（近期、未來或缺少日期仍最多六小時）；桌面已完結詳情最多七天。伺服器另有播出日感知快取，因此上游修正可能需要經過各層回查才顯示；查詢失敗或資料不完整時不宣稱已確認最新播出總數。
