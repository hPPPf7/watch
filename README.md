...
- **資料待確認**：整季尚未載入、查詢失敗或集數資料不一致時保留已看集數；補齊後自動顯示進度。單集未定播出日期則不計入分母，不會讓整部作品待確認。
...
- 完整集名清單的前端快取：播出中六小時，已完結舊季度七天（近期、未來或缺少日期仍最多六小時）；桌面已完結詳情最多七天。伺服器另有播出日感知快取，因此上游修正可能需要經過各層回查才顯示；缺少播出日期的已載入集數不計入，日期摘要也會在本機保留最多六小時，避免反覆補查；整季未載入、查詢失敗或集數不一致時仍顯示待確認。
